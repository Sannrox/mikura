# Code of conduct

## Our commitment

Participation in kura should be a respectful, harassment-free experience for
everyone, regardless of background, identity, experience level, or personal
characteristics.

## Expected behavior

- Be respectful, specific, and constructive.
- Critique ideas and implementations, not people.
- Assume good intent while still addressing harmful impact.
- Respect privacy, consent, and requests to stop an unwanted interaction.
- Help make technical discussions accessible to newcomers.
- Disclose conflicts of interest that could affect a project decision.

## Unacceptable behavior

- Harassment, threats, intimidation, stalking, or discriminatory language.
- Sexualized language, imagery, or unwanted attention.
- Personal attacks, insults, trolling, or sustained disruption.
- Publishing another person's private information without explicit permission.
- Deliberately exposing credentials, private data, or vulnerability details.
- Retaliation against someone who raises a good-faith concern.

## Scope

This code applies in repository issues, pull requests, reviews, discussions,
and other project spaces. It also applies when someone officially represents
the project in public.

## Reporting and enforcement

Report conduct concerns privately to the repository maintainers. Do not open
a public issue that identifies the people involved. Use the process in
[SECURITY.md](SECURITY.md) only when a report also involves an exploitable
vulnerability or sensitive security data.

Maintainers may warn, restrict, or remove access when this code is violated.
