...
| v1 adapter | sekai-chisei `SEKAI_OBJECT_STORE=sql\|dual\|kura` (default **sql**). Dual-read fail closed. SQL index **not** deleted (ADR 0081 in the control-plane repo) |

## Next (in order)

1. **Promote the log.** Replace JSONL in `Store` with paged + group-commit from spikes 003–005. Format ADR in this repo.
...
- No merging this git repo into sekai-chisei; the control plane may **depend** on the kura crate (copy or git dep)
