...
| **Schema descriptor** | Clerk-authored kind/property/link rules persisted as `mikura.schema/<kind>` (`properties`, `required`, `links`, optional `sums`). `sums` names last-hop measure properties already in `properties`. Visible writes of that kind fail closed on unknown, missing required, or empty outgoing-link keys. `Store::load` still returns historical unvalidated rows; `Store::load_with_schema` applies a supplied check. Not a query language. [ADR 0008](decisions/0008-type-link-delete.md), [ADR 0010](decisions/0010-last-hop-measures.md). Optional `types` (boolean, integer, timestamp, decimal) is accepted in [ADR 0012](decisions/0012-typed-values.md) and not implemented until [#168](https://github.com/Sannrox/mikura/issues/168). |
