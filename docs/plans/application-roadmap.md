...
| M3 — refresh-safe edits (contract accepted) | Property overlay as `mikura.overlay` objects; `hidden` delete; expected-generation stale write ([ADR 0009](../decisions/0009-refresh-safe-edit-overlay.md)) | Implementation still open. Exit: edit → source refresh → retry → crash/reopen → sidecar delete preserves `note=acked` on `inc-1`; stale expected generation fails closed |
