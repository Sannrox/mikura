...
5. Open the Pull Request as a draft with the first published commit, then mark
   ...
   - may list public lane fields only: claim branch, base SHA, published SHA,
     authority ceiling. Never hostname, FQDN, home path, absolute worktree
     path, LAN or employer network name, or other environment inventory;
