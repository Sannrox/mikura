...
impl SyncPolicy {
    fn parse(sync: &str, group: u32) -> Result<Self, String> {
        match sync {
            "none" => Ok(Self::None),
            "page" => Ok(Self::Page),
            "group" => {
            ...
            other => Err(format!("unknown --sync {other}; expected none|page|group")),
        }
    }
...
impl Writer {
    ...
    fn finish(mut self) -> Result<(u32, u32), String> {
        self.flush_page()?;
        self.commit_pending()?;
        self.file.flush().map_err(|e| e.to_string())?;
        Ok((self.written, self.committed))
    }
}
...
fn main() -> ExitCode {
    ...
    let group = flag(&args, "--group")
        .and_then(|v| v.parse().ok())
        .unwrap_or(32);
    let sync = match SyncPolicy::parse(
        &flag(&args, "--sync").unwrap_or_else(|| "group".into()),
        group,
    ) {
        Ok(sync) => sync,
        Err(error) => {
    ...
    let (written, committed) = match funnel_snapshot(&log_path, &scale, sync) {
        Ok(pair) => pair,
        Err(error) => {
    ...
    println!("spike=005-group-commit");
    println!("sync={}", sync.as_str());
    println!("objects={}", scale.total());
    println!("written_pages={written}");
    ...
    let _ = fs::remove_dir_all(&dir);
    if identity_hold && hop > 0 && written == committed {
        ExitCode::SUCCESS
    } else {
        ExitCode::from(1)
    }
...
fn funnel_snapshot(path: &Path, scale: &Scale, sync: SyncPolicy) -> Result<(u32, u32), String> {
...
mod tests {
    ...
    fn uncommitted_group_tail_is_not_authority() {
        ...
        let (written, committed) =
            funnel_snapshot(&path, &scale, SyncPolicy::Group(64)).unwrap();
        ...
        let after = rebuild(&path).unwrap();
        assert_eq!(identity_digest(&live), identity_digest(&after));
