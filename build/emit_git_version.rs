...
pub fn emit() {
    ...
    // MIKURA_GIT_* overrides, not every unstaged file in the repository.
    println!("cargo:rerun-if-env-changed=MIKURA_GIT_VERSION");
    println!("cargo:rerun-if-env-changed=MIKURA_GIT_COMMIT");
    ...
    let version = first_nonempty(&[
        env_nonempty("MIKURA_GIT_VERSION"),
        git(&["describe", "--tags", "--always", "--dirty"]),
    ])
    .unwrap_or(pkg);
    let commit = first_nonempty(&[
        env_nonempty("MIKURA_GIT_COMMIT"),
        git(&["rev-parse", "HEAD"]),
    ])
    .unwrap_or_else(|| "unknown".to_string());
    println!("cargo:rustc-env=MIKURA_GIT_VERSION={version}");
    println!("cargo:rustc-env=MIKURA_GIT_COMMIT={commit}");
}
